...
bool vote(int rank, string name, int ranks[])
{
    // Verifica se o nome corresponde a um candidato válido
    for (int i = 0; i < candidate_count; i++)
    {
        if (strcmp(name, candidates[i]) == 0)
        {
            // ranks[i] is voter's ith preference
            ranks[rank] = i;
            return true; // Votação bem-sucedida
        }
    }
    return false;
}
...
void sort_pairs(void)
{
    ...
    for (int i = 0; i < pair_count - 1; i++)
    {
        ...
        {
            // BubbleSort, Compare pairs based on indexed values in decreasing order
...
void print_winner(void)
{
    for (int col = 0; col < candidate_count; col++)
    {
        ...
        for (int row = 0; row < candidate_count; row++)
        {
            ...
            {
                false_count++;
                if (false_count == candidate_count) // No one has won against the candidate
                {
                    printf("%s\n", candidates[col]);
                    break;
                }
